						 ...
						 const std::string& button1title, const std::string& button2title) : _buttonFlags(buttons)
{
	...
	_button2 = Button::Pointer(new Button(button1title, font, _background.ptr()));
...
{
	vec2 bgSize = maxv(_background->imageDescriptor().size, floorv(0.333f * sz));
	...
	_button1->setPosition(floorv(b1pos));
	_button2->setPosition(floorv(b2pos));
